...
geometry = {112,92}--,112}--{184, 224}--{92,112}
...
if opt.network == '' then
   ...
   if opt.model == 'convnet' then
      ...
      model:add(nn.SpatialConvolutionMM(1, 32, 3, 3, 1, 1, 1, 1))   -- 32x184x224
      model:add(nn.LeakyReLU())
      model:add(nn.SpatialConvolutionMM(32, 32, 3, 3, 1, 1, 1, 1))   -- 32x184x224
      model:add(nn.LeakyReLU())
      model:add(nn.SpatialMaxPooling(2, 2, 2, 2))                   -- 32x92x112

      model:add(nn.SpatialConvolutionMM(32, 64, 3, 3, 1, 1, 1, 1))  -- 64x92x112
      model:add(nn.LeakyReLU())
      model:add(nn.SpatialConvolutionMM(64, 64, 3, 3, 1, 1, 1, 1))  -- 64x92x112
      model:add(nn.LeakyReLU())
      model:add(nn.SpatialMaxPooling(2, 2, 2, 2))                    -- 64x46x56

      model:add(nn.SpatialConvolutionMM(64, 128, 3, 3, 1, 1, 1, 1))  -- 128x46x56
      model:add(nn.LeakyReLU())
      model:add(nn.SpatialConvolutionMM(128, 128, 3, 3, 1, 1, 1, 1))  -- 128x46x56
      model:add(nn.LeakyReLU())
      model:add(nn.SpatialMaxPooling(2, 2, 2, 2))                     -- 128x23x27

      model:add(nn.SpatialConvolutionMM(128, 256, 3, 3, 1, 1, 1, 1))  -- 256x23x27
      model:add(nn.LeakyReLU())
      model:add(nn.SpatialConvolutionMM(256, 256, 3, 3, 1, 1, 1, 1))  -- 256x23x27
      model:add(nn.LeakyReLU())
      model:add(nn.SpatialMaxPooling(2, 2, 2, 2))                     -- 256x11x14

      model:add(nn.SpatialConvolutionMM(256, 512, 3, 3, 1, 1, 1, 1))  -- 512x23x27
      model:add(nn.LeakyReLU())
      model:add(nn.SpatialConvolutionMM(512, 512, 3, 3, 1, 1, 1, 1))  -- 512x23x27
      model:add(nn.LeakyReLU())
      model:add(nn.SpatialMaxPooling(2, 2, 2, 2))                   -- 512x5x7
      local MLP = 512*5*7

      -- MLP
      model:add(nn.Reshape(MLP))
      model:add(nn.Linear(MLP, 1024))
      model:add(nn.LeakyReLU())
      ...
      model:add(nn.SpatialConvolutionMM(64, 128, 3, 3, 1, 1, 1, 1)) -- 128x46x56
      model:add(nn.LeakyReLU())
      model:add(nn.SpatialMaxPooling(3, 3, 3, 3))                   -- 128x23x28
      local MLP = 128*15*18
...
-- get/create dataset
--
nbTrainingPatches = 5*8*40
nbTestingPatches = 5*2*40
...
mnist.setvalues(opt.train, "", opt.test,
                nbTrainingPatches, 0, nbTestingPatches)
...
function train()
   ...
   local shuffle = torch.randperm(mnist.trainData.size)

   -- do one epoch
   print("\n")
   print('<trainer> on training set:')
   print("<trainer> online epoch # " .. epoch .. ' [batchSize = ' .. opt.batchSize .. ']')
   for t = 1, mnist.trainData.size,opt.batchSize do

      -- create mini batch
      local inputs = torch.Tensor(opt.batchSize,1,geometry[1],geometry[2])
      local targets = torch.Tensor(opt.batchSize)
      local k = 1
      for i = t,math.min(t+opt.batchSize-1, mnist.trainData.size) do
         -- load new sample
         local sample = mnist.loadsingle("train", shuffle[i], true)
         --print(sample.image:size())
         --print(inputs:size())
         inputs[k] = sample.image         
         targets[k] = sample.label
         k = k + 1
      end
      ...
      if opt.optimization == 'LBFGS' then
         ...
         -- disp report:
         print('LBFGS step')
         print(' - progress in batch: ' .. t .. '/' .. mnist.testData.size)
      ...
      elseif opt.optimization == 'SGD' then
         ...
         if opt.progress then
            xlua.progress(t, mnist.trainData.size)
         end
   ...
   if opt.progress then
      xlua.progress(mnist.trainData.size, mnist.trainData.size)
   end
   
   -- time taken
   time = sys.clock() - time
   time = time / mnist.trainData.size
...
function test()
   ...
   for t = 1,mnist.testData.size,opt.batchSize do
      -- create mini batch
      local inputs = torch.Tensor(opt.batchSize,1,geometry[1],geometry[2])
      local targets = torch.Tensor(opt.batchSize)
      local k = 1
      for i = t,math.min(t+opt.batchSize-1,mnist.testData.size) do
         -- load new sample
         local sample = mnist.loadsingle('test', i, true)
         inputs[k] = sample.image
         targets[k] = sample.label
         k = k + 1
      end
      ...
      -- disp progress
      if opt.progress then
         xlua.progress(t, mnist.testData.size)
      end
   end

   -- These unfinished progress bars are annoying me...
   if opt.progress then
      xlua.progress(mnist.testData.size, mnist.testData.size)
   end


   -- timing
   time = sys.clock() - time
   time = time / mnist.testData.size
   ...
   print("Total Valid: " .. totalValid .. "%")
   for i = 1,opt.top do
      print("Correct in top " .. i .. ": " .. top[i]/mnist.testData.size*100 .. "%") 
...
while true do
   ...
   --print(m)
   for i = 1,32 do
      image.save("filters/filter".. i .. ".png", m[4].output[1][i])--:transpose(1,2))
   end
