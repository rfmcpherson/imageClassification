...
def condor(label, data_dir):
    ...
    script += "exec th "+path+"torch-scripts/train-on-cifar.lua \\\n" 
    script += "--full \\\n" 
    script += "-b 1000 \\\n" 
    script += "--save "+res_dir+label+" \\\n"
    script += "--top 5 \\\n" 
    script += "--weightDecay 5e-4 \\\n" 
    script += "--momentum 0.9 \\\n" 
    script += "--dropout 0.5 \\\n"
    script += "--learningRate 1e-2 \\\n"
    script += "--infolder "+data_dir
...
def main():
    ...
    # Normal images
    #condor("norm", path+"cifar-data/norm/")

    
    # P3 images
    condor("p3-1", path+"cifar-data/p3-1/")
    condor("p3-10", path+"cifar-data/p3-10/")
    condor("p3-20", path+"cifar-data/p3-20/")

    # Pixelated images
    condor("pix-4", path+"cifar-data/pix-4/")
    condor("pix-8", path+"cifar-data/pix-8/")
    condor("pix-16", path+"cifar-data/pix-16/")
